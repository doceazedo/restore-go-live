...
if (!BUNDLES || !existsSync(BUNDLES)) {
  console.error(
    "usage: node scripts/checkpatches.mjs <dir-of-discord-js-bundles>",
  );
  process.exit(2);
}
...
const src = ["src/patches.ts", "src/videoGuard.ts"]
...
for (const p of patches) {
  ...
  for (const f of files) {
    ...
    const patched = fn.replace(
      re,
      p.replace.replaceAll("$self", "globalThis.__self"),
    );
    try {
